			...
			case 'musicroom':
				{
					...
					if (SONG.song.toLowerCase().startsWith('love n funkin'))
					{
						...
						// picohandoatlas
						var poemSprite:String = (SONG.song.toLowerCase().endsWith('-alt') ? 'assets/doki/images/notepad/picohandoatlas' : 'assets/doki/images/notepad/handoatlas');
						poemVideo = new FlxAnimate(0, 0, poemSprite);
			...
			case 'libitina':
				{
					...
					if (!SaveData.lowEnd)
					{
						...
						testVM.load(Paths.video('testvm'));
						testVM.play();
					...
					if (!SaveData.lowEnd)
					{
						...
						crackBG.load(Paths.video('crackBG'));
						crackBG.play();
